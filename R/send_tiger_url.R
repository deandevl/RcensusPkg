...
#' @param output_dir A required parameter that sets the full directory path for the download
#'   shapefile destination. The files in this directory are removed before the download.
...
.send_tiger_url <- function(
  ...
  if(!file.exists(output_dir) | is.null(output_dir)){
  ...
  files <- list.files(output_dir, include.dirs = TRUE, full.names = TRUE, recursive = TRUE)
  file.remove(files)
