...
ca_urban_pop_dt <- RcensusPkg::get_vintage_data(
  ...
  _[, population := as.numeric(population)] |>
  _[population >= 750000 & grepl(pattern = "CA Urbanized Area (2010)", NAME, fixed = T),] |>
  _[, NAME := stringr::str_remove(NAME, stringr::fixed(" Urbanized Area (2010)"))]
...
ca_urban_pop_sf <- RcensusPkg::join_it(
  ...
  _[, NAME := stringr::str_remove(NAME, fixed(", CA"))] |>
  _[, Label := paste(NAME,population)] |>
  sf::st_as_sf()
