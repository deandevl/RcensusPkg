...
Some preliminaries: Note that an existing local folder must be specified to receive all downloaded Census Tiger related shapefiles. We will use the default directory given by `tempdir()`. Also when requesting geometries we can define an "express" parameter to limit the request with a logical expression. 
```{r}
...
computers_present_2013_compare_plot <- RspatialPkg::get_geom_sf(
  sf = state_boundaries_2013_sf,
  aes_fill = "ComputerPresent",
  hide_x_tics = TRUE,
  hide_y_tics = TRUE,
  legend_pos = "top",
  legend_key_width = 1.4
) +
ggplot2::scale_fill_gradientn(
  colors = RColorBrewer::brewer.pal(n = 9, name = "Greens"),
  breaks = seq(from = 60, to = 100, by = 5),
...
computers_present_2021_compare_plot <- RspatialPkg::get_geom_sf(
  sf = state_boundaries_2021_sf,
  aes_fill = "ComputerPresent",
  hide_x_tics = TRUE,
  hide_y_tics = TRUE,
...
plot_lst <- list(computers_present_2013_compare_plot, computers_present_2021_compare_plot)
